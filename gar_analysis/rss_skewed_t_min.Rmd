...
# One Quarter ahead Parameter Estimation
  ...
  for(i in 1:nrow(q_predict_1_quart)){
    print(paste("Estimating case ", i, " out of ", nrow(q_predict_1_quart)))
    q_hat <- c(q_predict_1_quart$'0.05'[i], q_predict_1_quart$'0.25'[i], q_predict_1_quart$'0.75'[i], q_predict_1_quart$'0.95'[i])
    sol <- 1e10
    for (f in 1:30){
      opt <- optim(c(0,1,0),
                   RSS_Loss, 
